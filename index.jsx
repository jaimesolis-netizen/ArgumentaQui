...
// ═══════════════════════════════════════════════════════
// IA — llamada al Worker (OpenRouter sin exponer la key)
// ═══════════════════════════════════════════════════════
const OR_MODEL = "meta-llama/llama-3.3-70b-instruct";

const callAI = async (system, user, maxTokens=1000) => {
  if (!cfg.workerUrl) throw new Error("Worker URL no configurado");
...
// ═══════════════════════════════════════════════════════
// FIREBASE FIRESTORE REST — sin SDK, sin dependencias
// ═══════════════════════════════════════════════════════
const fsBase = () =>
...
{"nivel":"Básico|Intermedio|Avanzado","estrellas":1-5,"fortaleza":"1-2 oraciones","mejora":"sugerencia concreta 1-2 oraciones","pregunta":"pregunta socrática"}`;
      ...
      setAiFb({nivel:"—",estrellas:0,fortaleza:e.message||"Error al obtener retroalimentación. Verifica la URL del Worker.",mejora:"Verifica tu conexión e intenta de nuevo.",pregunta:""});
    }
    setLoadingAI(false);
  };
...
${JSON.stringify(payload,null,2)}`;
    ...
    } catch(e) { setErr(`No se pudo generar el análisis: ${e.message||"verifica tu conexión e intenta de nuevo."}`); }
    setLoading(false);
  };
